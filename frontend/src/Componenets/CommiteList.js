...
const COMMITTEE_ROLES = [
  { id: 3, label: 'President' },
  { id: 4, label: 'Vice President' },
  { id: 5, label: 'Treasurer' },
  { id: 6, label: 'Normal Member' },
];
...
const ROLE_COLORS = {
  ...
  'Normal Member': 'bg-gray-100 text-gray-600',
};

const CSV_TEMPLATE = 'fullName,email,password,roleId\nAhmed President,ahmed@esi.dz,Pass@1234,3\nSara Member,sara@esi.dz,Pass@1234,6';
...
const downloadTemplate = () => {
  ...
  const a = document.createElement('a'); a.href = url; a.download = 'committee_template.csv'; a.click();
...
const CommiteList = () => {
  ...
  const [createForm, setCreateForm] = useState({ fullName: '', email: '', password: '', roleId: 6 });
  ...
  const handleCreate = async e => {
    ...
    try {
      ...
      setCreateForm({ fullName: '', email: '', password: '', roleId: 6 });
      setShowCreate(false);
      fetchMembers();
    } catch (err) {
      setCreateError(err.response?.data?.error || 'Failed to create member.');
    } finally {
      setCreating(false);
    }
  };
  ...
  const openEdit = m => {
    setEditUser(m);
    setEditForm({ fullName: m.User?.fullName || '', email: m.User?.email || '' });
    setEditError('');
  };
  ...
  const Avatar = ({ name, roleName }) => (
    <div className={`w-8 h-8 rounded-full flex items-center justify-center font-bold text-sm flex-shrink-0 ${ROLE_COLORS[roleName] || 'bg-gray-100 text-gray-500'}`}>
      {name?.[0]?.toUpperCase() || '?'}
    </div>
  );
  ...
  return (
    <div className="flex flex-col w-full h-full">
      <div className="w-full flex flex-row py-4 justify-between pl-6 pr-6 items-center shadow-sm bg-white">
        <div className="text-xl font-bold text-dblue">Committee Members</div>
      ...
      <div className="bg-secondary w-full flex-1 overflow-y-auto px-6 py-6">
        <div className="bg-white rounded-2xl p-5 flex flex-col gap-4">
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                {loading ? (
                  ...
                  <tr><td colSpan={5} className="text-center py-10 text-gray-400">No committee members found.</td></tr>
                ) : filtered.map((m, i) => (
                  <tr key={m.id} className="border-b border-gray-50 hover:bg-gray-50 transition-colors">
                    <td className="px-4 py-3 text-gray-400">{i + 1}</td>
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-2">
                        <Avatar name={m.User?.fullName} roleName={m.UserRole?.roleName} />
                        <span className="font-medium text-gray-700">{m.User?.fullName || '—'}</span>
                      </div>
                    </td>
                    <td className="px-4 py-3 text-gray-500">{m.User?.email || '—'}</td>
                    <td className="px-4 py-3">
                      <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${ROLE_COLORS[m.UserRole?.roleName] || 'bg-gray-100 text-gray-500'}`}>
                        {m.UserRole?.roleName || '—'}
                      </span>
                    </td>
                    <td className="px-4 py-3">
                      <div className="flex items-center justify-center gap-2">
                        <button onClick={() => setViewUser(m)} className="w-8 h-8 rounded-lg flex items-center justify-center text-blue-500 hover:bg-blue-50 transition-colors" title="View">
                          <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth="1.5" stroke="currentColor" className="size-4">
                            <path strokeLinecap="round" strokeLinejoin="round" d="M2.036 12.322a1.012 1.012 0 0 1 0-.639C3.423 7.51 7.36 4.5 12 4.5c4.638 0 8.573 3.007 9.963 7.178.07.207.07.431 0 .639C20.577 16.49 16.64 19.5 12 19.5c-4.638 0-8.573-3.007-9.963-7.178Z" />
                            <path strokeLinecap="round" strokeLinejoin="round" d="M15 12a3 3 0 1 1-6 0 3 3 0 0 1 6 0Z" />
                          </svg>
                        </button>
                        <button onClick={() => openEdit(m)} className="w-8 h-8 rounded-lg flex items-center justify-center text-green-500 hover:bg-green-50 transition-colors" title="Edit">
                          <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth="1.5" stroke="currentColor" className="size-4">
                            <path strokeLinecap="round" strokeLinejoin="round" d="m16.862 4.487 1.687-1.688a1.875 1.875 0 1 1 2.652 2.652L6.832 19.82a4.5 4.5 0 0 1-1.897 1.13l-2.685.8.8-2.685a4.5 4.5 0 0 1 1.13-1.897L16.863 4.487Zm0 0L19.5 7.125" />
                          </svg>
                        </button>
                        <button onClick={() => setDeleteTarget(m)} className="w-8 h-8 rounded-lg flex items-center justify-center text-red-500 hover:bg-red-50 transition-colors" title="Delete">
                          <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth="1.5" stroke="currentColor" className="size-4">
          ...
          <p className="text-xs text-gray-400">{filtered.length} member{filtered.length !== 1 ? 's' : ''} found</p>
        </div>
      </div>
      ...
      {showCreate && (
        <div className="fixed inset-0 flex items-center justify-center bg-black/30 z-50">
          <div className="bg-white w-full max-w-md mx-4 rounded-2xl p-6 flex flex-col gap-4 relative">
            ...
            <h2 className="text-lg font-bold text-dblue">Create Committee Member</h2>
            <form onSubmit={handleCreate} className="flex flex-col gap-3">
              <div className="flex flex-col gap-1">
                <label className="text-sm font-medium text-gray-600">Full Name</label>
                <input type="text" value={createForm.fullName} onChange={e => setCreateForm({ ...createForm, fullName: e.target.value })} required placeholder="e.g. Ahmed Benali" className="border border-gray-200 rounded-xl px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-pblue/30" />
              </div>
              <div className="flex flex-col gap-1">
                <label className="text-sm font-medium text-gray-600">Email</label>
                <input type="email" value={createForm.email} onChange={e => setCreateForm({ ...createForm, email: e.target.value })} required placeholder="user@esi-sba.dz" className="border border-gray-200 rounded-xl px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-pblue/30" />
              </div>
              <div className="flex flex-col gap-1">
                <label className="text-sm font-medium text-gray-600">Password</label>
                <input type="password" value={createForm.password} onChange={e => setCreateForm({ ...createForm, password: e.target.value })} required placeholder="Min. 8 characters" className="border border-gray-200 rounded-xl px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-pblue/30" />
              </div>
              <div className="flex flex-col gap-1">
                <label className="text-sm font-medium text-gray-600">Committee Role</label>
                <div className="grid grid-cols-2 gap-2">
                  {COMMITTEE_ROLES.map(r => (
                    <button
                      key={r.id}
                      type="button"
                      onClick={() => setCreateForm({ ...createForm, roleId: r.id })}
                      className={`py-2 px-3 rounded-xl text-sm font-medium border transition-all ${createForm.roleId === r.id ? 'bg-dblue text-white border-dblue' : 'border-gray-200 text-gray-600 hover:bg-gray-50'}`}
                    >
                      {r.label}
                    </button>
                  ))}
      ...
      {showImport && (
        <div className="fixed inset-0 flex items-center justify-center bg-black/30 z-50">
          <div className="bg-white w-full max-w-xl mx-4 rounded-2xl p-6 flex flex-col gap-4 relative max-h-[90vh] overflow-y-auto">
            ...
            <h2 className="text-lg font-bold text-dblue">Import Committee Members</h2>
            <div className="bg-blue-50 rounded-xl p-4 text-sm text-dblue flex flex-col gap-1">
              <p className="font-semibold">Required columns:</p>
              <code className="text-xs bg-white rounded px-2 py-1 font-mono">fullName, email, password, roleId</code>
              <p className="text-xs text-gray-500 mt-1">roleId: 3=President, 4=VP, 5=Treasurer, 6=Normal Member</p>
            ...
            {importRows.length > 0 && (
              <div className="flex flex-col gap-2">
                <p className="text-sm font-semibold text-dblue">{importRows.length} row{importRows.length !== 1 ? 's' : ''} found — preview:</p>
                  ...
                  <table className="w-full text-xs">
                    <thead className="bg-gray-50">
                      <tr>
                        ...
                        <th className="px-3 py-2 text-left text-gray-500 font-semibold">Role ID</th>
                      </tr>
                    </thead>
                    <tbody>
                      {importRows.map((r, i) => (
                        <tr key={i} className="border-t border-gray-100">
                          ...
                          <td className="px-3 py-2 text-gray-500">{r.roleId || '6'}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
                ...
                >
                  {importing ? importProgress : `Import ${importRows.length} Member${importRows.length !== 1 ? 's' : ''}`}
                </button>
              </div>
            )}
          </div>
        ...
        <div className="fixed inset-0 flex items-center justify-center bg-black/30 z-50">
          <div className="bg-white w-full max-w-sm mx-4 rounded-2xl p-6 flex flex-col gap-4 relative">
            ...
            <div className="flex flex-col items-center gap-2 mb-2">
              <div className={`w-16 h-16 rounded-full flex items-center justify-center font-bold text-2xl ${ROLE_COLORS[viewUser.UserRole?.roleName] || 'bg-gray-100 text-gray-500'}`}>
                {viewUser.User?.fullName?.[0]?.toUpperCase() || '?'}
              </div>
              <h2 className="text-lg font-bold text-dblue">{viewUser.User?.fullName}</h2>
              <span className={`text-xs px-3 py-1 rounded-full font-medium ${ROLE_COLORS[viewUser.UserRole?.roleName] || 'bg-gray-100 text-gray-500'}`}>
                {viewUser.UserRole?.roleName || '—'}
              </span>
            </div>
            <div className="flex flex-col gap-2 text-sm bg-gray-50 rounded-xl p-4">
              <div className="flex gap-2"><span className="font-semibold text-gray-500 w-24">Email:</span><span className="text-gray-700">{viewUser.User?.email}</span></div>
              <div className="flex gap-2"><span className="font-semibold text-gray-500 w-24">Role type:</span><span className="text-gray-700">{viewUser.UserRole?.roletype}</span></div>
              <div className="flex gap-2"><span className="font-semibold text-gray-500 w-24">Account ID:</span><span className="text-gray-700">{viewUser.id}</span></div>
            </div>
          </div>
        </div>
      )}
      ...
      {editUser && (
        <div className="fixed inset-0 flex items-center justify-center bg-black/30 z-50">
          <div className="bg-white w-full max-w-md mx-4 rounded-2xl p-6 flex flex-col gap-4 relative">
            ...
            <h2 className="text-lg font-bold text-dblue">Edit Committee Member</h2>
      ...
      {deleteTarget && (
        <div className="fixed inset-0 flex items-center justify-center bg-black/30 z-50">
          <div className="bg-white w-full max-w-sm mx-4 rounded-2xl p-6 flex flex-col gap-4">
            <h2 className="text-lg font-bold text-dblue">Remove Member</h2>
            <p className="text-sm text-gray-600">
              Are you sure you want to remove <strong>{deleteTarget.User?.fullName}</strong> from the committee? This cannot be undone.
            </p>
