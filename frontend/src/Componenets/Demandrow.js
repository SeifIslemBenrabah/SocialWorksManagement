...
const Demandrow = ({ name, Programme, date, state, img }) => {
  ...
  return (
    <div className='flex flex-row justify-between items-center w-full space-x-4 border-[0.4px] rounded-md border-gray-300 p-2'>
      ...
      <div className='flex flex-row w-9/12 justify-between'>
        ...
        <p className='whitespace-nowrap'>{Programme}</p>
        <p className='whitespace-nowrap'>{date}</p>
        {/* Replace status text with a colored div */}
        <div className={`text-white text-xs px-3 py-1 rounded-full ${getStatusColor(state)}`}>
          {state}
        </div>
      </div>
      <button className='p-2 bg-pblue text-white rounded-lg'>
